...
# audio processor for call routing 
...
class AudioProcessor:
    ...
    def process_twilio_stream(self, base64_payload: str) -> torch.Tensor:
        """Decodes Twilio's μ-law base64 stream to a resampled PCM Tensor."""
        # 1. Decode base64 to raw bytes
        raw_data = base64.b64decode(base64_payload)
        # 2. Convert μ-law (8-bit) to PCM (16-bit)
        pcm_data = audioop.ulaw2lin(raw_data, 2)
        # 3. Convert to Tensor
        waveform = torch.from_numpy(np.frombuffer(pcm_data, dtype=np.int16).astype(np.float32))
        waveform = waveform.unsqueeze(0) / 32768.0 # Normalize
        # 4. Resample from 8k to 16k
    ...
    def process_multiple_chunks(self, audio_bytes: bytes, suffix: str = ".wav") -> dict:
        ...
        Applies the full preprocessing pipeline (high-pass filter,
        pre-emphasis, normalization) to the WHOLE waveform BEFORE splitting.
        """
        waveform, sample_rate = self._load_with_av(audio_bytes)
        ...
        # 3. High-pass filter (remove noise below 80Hz)
        waveform = torchaudio.functional.highpass_biquad(waveform, self.target_sr, 80.0)
        
        # 4. Pre-emphasis (critical for AASIST)
        waveform = waveform - 0.97 * torch.cat([waveform[:, :1], waveform[:, :-1]], dim=1)
        
        # 5. Normalize the whole waveform
        if waveform.abs().max() > 0:
            waveform = waveform / waveform.abs().max()
        ...
        if not chunks:
            chunk = self._pad_or_trim(waveform)
            chunks = [chunk.squeeze(0)]
